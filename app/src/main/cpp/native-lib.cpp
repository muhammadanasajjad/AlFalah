...
static int CallJavaGetReciterCount() {
    ...
    if (clazz) {
        jmethodID method =
            env->GetStaticMethodID(clazz, "getReciterCount", "()I");
...
static std::string CallJavaGetReciterName(int index) {
    ...
    std::string name;
    if (clazz) {
        jmethodID method = env->GetStaticMethodID(clazz, "getReciterName",
                                                  "(I)Ljava/lang/String;");
        if (method) {
            jstring jName =
                (jstring)env->CallStaticObjectMethod(clazz, method, index);
            if (jName) {
...
static void CallJavaTogglePlayPause() {
    ...
    if (clazz) {
        jmethodID method =
            env->GetStaticMethodID(clazz, "togglePlayPause", "()V");
...
// ── Mushaf paged horizontal scroll ──────────────────────────────────────────
static int g_mushafPageCount = 0;
static int g_mushafCurrentPage = 0;  // 0-based index into displayed pages
static float g_mushafScrollX = 0.f;  // current animated X offset in dp
static float g_mushafTargetX = 0.f;  // target X offset we're snapping to
static float g_mushafPageStride =
    0.f;  // width of each page panel in dp (set each frame)
static float g_swipeStartX = 0.f;
static float g_swipeDeltaX = 0.f;
static bool g_isSwiping = false;

static Clay_Dimensions MeasureText(Clay_StringSlice text,
                                   Clay_TextElementConfig* config, void*) {
...
Java_com_primaveradev_alfalah_MainActivity_nativeOnSurfaceCreated(
...

    {
        ...
        for (float dp : dpSizes) {
            uint16_t fid =
                g_fontManager.LoadSystem("Roboto", dp * g_density, false);
    ...
    if (!g_clayArena) {
        ...
        Clay_Initialize(
            ...
            (Clay_Dimensions){0, 0}, (Clay_ErrorHandler){0});
    }
...
extern "C" JNIEXPORT void JNICALL
Java_com_primaveradev_alfalah_MainActivity_nativeOnSurfaceChanged(JNIEnv*,
                                                                  jobject,
                                                                  jint w,
                                                                  jint h) {
    ...
    Clay_SetLayoutDimensions(
        (Clay_Dimensions){g_screenWidthDp, g_screenHeightDp});
}
...
Java_com_primaveradev_alfalah_MainActivity_nativeOnPlaybackStateChanged(
    JNIEnv*, jclass clazz, jboolean active, jboolean paused, jint surah,
    jint ayah) {
    g_isPlaying = active;
    g_isPaused = paused;
...
extern "C" JNIEXPORT void JNICALL
Java_com_primaveradev_alfalah_MainActivity_nativeOnTouch(JNIEnv*, jobject,
                                                         jfloat x, jfloat y,
                                                         jboolean down) {
    float xDp = x / g_density;
    float yDp = y / g_density;
    ...
    if (down && !g_pointerDown) {
        ...
        // Begin swipe tracking for mushaf paged mode
    ...
    // Live drag: shift the scroll offset so the page follows the finger
    if (g_isSwiping && g_currentPage == Page::Quran &&
        g_quranDisplayMode == QuranDisplayMode::Mushaf) {
        g_swipeDeltaX = xDp - g_swipeStartX;
        float base = g_mushafCurrentPage * g_mushafPageStride;
        g_mushafScrollX = base - g_swipeDeltaX;
    }
    ...
    // Finger lifted — decide which page to snap to.
    // Arabic: left swipe (negative delta) = forward = next page (higher index).
    //         right swipe (positive delta) = backward = previous page (lower
    //         index).
    if (!down && g_isSwiping) {
        g_isSwiping = false;
        const float SNAP_THRESHOLD = 40.f;  // dp needed to commit a page turn

        if (g_swipeDeltaX > SNAP_THRESHOLD && g_mushafCurrentPage > 0) {
            // Swiped right → previous page (backward in Arabic reading order)
            --g_mushafCurrentPage;
        } else if (g_swipeDeltaX < -SNAP_THRESHOLD &&
                   g_mushafCurrentPage < g_mushafPageCount - 1) {
            // Swiped left → next page (forward in Arabic reading order)
            ++g_mushafCurrentPage;
        }
        g_mushafTargetX = g_mushafCurrentPage * g_mushafPageStride;
    }
}
...
static void LayoutHomePage() {
    CLAY(CLAY_ID("Root"),
             ...
             .layout =
                 {
                     ...
                     .padding = {.left = 24,
                                 .right = 24,
                                 .top =
                                     (uint16_t)(24 + (int)g_statusBarHeightDp),
                                 .bottom = 24},
                     .childGap = 24,
        ...
        CLAY(CLAY_ID("TopRowContainer"),
             {.layout = {
                  .sizing = {CLAY_SIZING_GROW(0)},
                  .childGap = 24,
                  .layoutDirection = CLAY_LEFT_TO_RIGHT,
              }}) {
            CLAY(CLAY_ID("QuranButtonContainer"),
                 {
                     .layout =
                         {
                ...
                CLAY(CLAY_ID("QuranText"),
                     {.layout = {
                          .padding = {.left = 24, .top = 24},
                          .childGap = 12,
                          .layoutDirection = CLAY_TOP_TO_BOTTOM,
                      }}) {
                    CLAY_TEXT(CLAY_STRING("Qur'an"),
                              CLAY_TEXT_CONFIG({
                                  .textColor = fg,
                                  .fontId = g_roboto24,
                                  .fontSize = 24,
                    ...
                    CLAY_TEXT(CLAY_STRING("Read and explore\nthe Qur'an"),
                              CLAY_TEXT_CONFIG({
                                  .textColor = fg,
                                  .fontId = g_roboto12,
                                  .fontSize = 12,
                ...
                CLAY(
                    CLAY_ID("ImageRow"),
                    {.layout = {
                         .sizing = {CLAY_SIZING_GROW(1), CLAY_SIZING_FIXED(70)},
                         .padding = {.left = 8},
                         .layoutDirection = CLAY_LEFT_TO_RIGHT,
                     }}) {
                    CLAY(CLAY_ID("QuranImage"),
                         {.layout = {.sizing =
                                         {
                                             CLAY_SIZING_PERCENT(1.20),
                                         }},
                          .cornerRadius = {8, 8, 8, 8},
                          .image = {.imageData = ImageLoader::Get(
                                        "images/quran.png")}}) {}
                }
            }

            CLAY(CLAY_ID("TasbihAlbumsContainer"),
                 {
                     .layout =
                         {
                             .sizing = {CLAY_SIZING_GROW(1),
                ...
                CLAY(CLAY_ID("TasbihContainer"),
                     {.layout =
                      ...
                      .cornerRadius = {24, 24, 24, 24},
                      .clip = {.horizontal = true, .vertical = true}}, ) {
                    CLAY(CLAY_ID("TasbihText"),
                         {.layout = {.padding = {.left = 24, .top = 24}}}) {
                        CLAY_TEXT(CLAY_STRING("Tasbih"),
                                  CLAY_TEXT_CONFIG({
                                      .textColor = fg,
                                      .fontId = g_roboto24,
                                      .fontSize = 24,
                ...
                CLAY(CLAY_ID("AlbumsContainer"),
                     {.layout =
                      ...
                      .cornerRadius = {24, 24, 24, 24},
                      .clip = {.horizontal = true, .vertical = true}}, ) {
                    CLAY(CLAY_ID("AlbumsText"),
                         {.layout = {.padding = {.left = 24, .top = 24}}}) {
                        CLAY_TEXT(CLAY_STRING("Albums"),
                                  CLAY_TEXT_CONFIG({
                                      .textColor = fg,
                                      .fontId = g_roboto24,
                                      .fontSize = 24,
        ...
        CLAY(CLAY_ID("TimesStatsContainer"),
             {
                 .layout =
                     {
                         .sizing = {CLAY_SIZING_GROW(1),
                                    CLAY_SIZING_FIXED((250 - 24) / 2)},
                         .childGap = 24,
                         .layoutDirection = CLAY_LEFT_TO_RIGHT,
                     },
             }) {
            CLAY(CLAY_ID("TimesContainer"),
                 {.layout =
                      {
                  ...
                  .cornerRadius = {24, 24, 24, 24},
                  .clip = {.horizontal = true, .vertical = true}}, ) {
                CLAY(CLAY_ID("TimesText"),
                     {.layout = {.padding = {.left = 24, .top = 24}}}) {
                    CLAY_TEXT(CLAY_STRING("Times"),
                              CLAY_TEXT_CONFIG({
                                  .textColor = fg,
                                  .fontId = g_roboto24,
                                  .fontSize = 24,
...
static void LayoutSurahSelectionPage() {
    ...
    if (!sBuilt) {
        ...
        for (int i = 0; i < 114; ++i) {
            ...
            surahDetails[i] =
                info.revelationPlace == "makkah" ? "Makki" : "Madani";
            surahDetails[i] +=
                " • " + std::to_string(info.verseCount) + " Verses";
            surahNamesStrings[i] = {.isStaticallyAllocated = false,
                                    .length = (int)surahNames[i].length(),
                                    .chars = surahNames[i].c_str()};
            surahNumbersStrings[i] = {.isStaticallyAllocated = false,
                                      .length = (int)surahNumbers[i].length(),
                                      .chars = surahNumbers[i].c_str()};
            surahDetailsStrings[i] = {.isStaticallyAllocated = false,
    ...
    CLAY(CLAY_ID("SurahSelectionRoot"),
         {
             .layout =
                 {
                     .sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)},
                     .padding = {.left = 16,
                                 .right = 16,
                                 .top =
                                     (uint16_t)(16 + (int)g_statusBarHeightDp),
                                 .bottom = 16},
                     .childGap = 12,
                     .layoutDirection = CLAY_TOP_TO_BOTTOM,
                 },
        ...
        CLAY(CLAY_ID("SurahSelectionTopBar"),
             {.layout = {
                  .sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_FIXED(48)},
                  .childGap = 12,
                  .layoutDirection = CLAY_LEFT_TO_RIGHT,
              }}) {
            CLAY(CLAY_ID("SurahBackButton"),
                 {
                     .layout =
                         {
                             .sizing = {CLAY_SIZING_FIXED(100),
            ...
            CLAY_TEXT(CLAY_STRING("Select Surah"),
                      CLAY_TEXT_CONFIG({
                          .textColor = fg,
                          .fontId = g_roboto22,
                          .fontSize = 22,
                ...
                CLAY(
                    ...
                    CLAY(CLAY_IDI("SurahNumber", i),
                         {.layout =
                          ...
                          .image = {.imageData = ImageLoader::Get(
                                        "images/starNew.png")}}) {
                        CLAY_TEXT(surahNumbersStrings[i],
                                  CLAY_TEXT_CONFIG({
                                      .textColor = fg,
                                      .fontId = g_roboto15,
                                      .fontSize = 15,
                    ...
                    CLAY(CLAY_IDI("SurahEnglishSide", i),
                         {
                             .layout =
                                 {
                                     .childGap = 6,
                        ...
                        CLAY_TEXT(surahDetailsStrings[i],
                                  CLAY_TEXT_CONFIG({
                                      .textColor = fg1,
                                      .fontId = g_roboto12,
                                      .fontSize = 12,
...
static void LayoutQuranTopBar() {
    CLAY(CLAY_ID("QuranTopBar"),
         {.layout = {
              .sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_FIXED(48)},
              .childGap = 12,
              .layoutDirection = CLAY_LEFT_TO_RIGHT,
          }}) {
        CLAY(CLAY_ID("QuranBackButton"),
             {
                 .layout =
                     {
                         .sizing = {CLAY_SIZING_FIXED(100),
        ...
        {  // reciter toggle
            ...
            if (s_lastRN != g_reciterName) {
                ...
                s_str = {false, (int)g_reciterName.length(),
                         g_reciterName.c_str()};
            }
            CLAY(CLAY_ID("QuranReciterToggle"),
                 {
                     .layout =
                         {
                             .sizing = {CLAY_SIZING_GROW(0),
                ...
                CLAY_TEXT(s_str, CLAY_TEXT_CONFIG({
                                     .textColor = accent,
                                     .fontId = g_roboto18,
                                     .fontSize = 18,
                                     .wrapMode = CLAY_TEXT_WRAP_WORDS,
        ...
        CLAY(CLAY_ID("QuranModeToggle"),
            ...
            CLAY_TEXT(sToggleLabel, CLAY_TEXT_CONFIG({
                                        .textColor = accent,
                                        .fontId = g_roboto18,
                                        .fontSize = 18,
                                        .wrapMode = CLAY_TEXT_WRAP_WORDS,
        ...
        CLAY(CLAY_ID("AudioStop"),
             {
                 ...
                 .image = {.imageData =
                               ImageLoader::Get("images/square-solid.png")},
             }) {}
        ...
        CLAY(CLAY_ID("AudioPrev"),
             {
                 ...
                 .image = {.imageData =
                               ImageLoader::Get("images/rewind-solid.png")},
             }) {}
        ...
        CLAY(CLAY_ID("AudioPlayPause"),
             {
                 .layout = {.sizing = {CLAY_SIZING_FIXED(28),
                                       CLAY_SIZING_FIXED(28)}},
                 .image = {.imageData = ImageLoader::Get(
                               g_isPaused ? "images/play-solid.png"
                                          : "images/pause-solid.png")},
             }) {}
        ...
        CLAY(CLAY_ID("AudioNext"),
             {
                 .layout = {.sizing = {CLAY_SIZING_FIXED(28),
                                       CLAY_SIZING_FIXED(28)}},
                 .image = {.imageData =
                               ImageLoader::Get("images/forward-solid.png")},
             }) {}
...
static void LayoutQuranStandardContent() {
    ...
    if (!sLoaded || g_extraStandardAyahs != s_loadedExtraAyahs) {
        if (!sLoaded) {
            ...
            for (auto& s : g_ayahTexts) {
                ayahStrings.push_back({.isStaticallyAllocated = false,
                                       .length = (int)s.length(),
                                       .chars = s.c_str()});
            }
            ...
            for (int32_t page : uniquePages) {
                char key[64];
                snprintf(key, sizeof(key), "fonts/quranicFonts/qpc/p%d.ttf",
                         page);
                uint16_t fid = g_fontManager.GetOrCreateFontId(
                    key, 16.0f * g_density, true);
                ...
                for (int i = 0; i < take; ++i) {
                    ...
                    if (page > 0) {
                        ...
                        snprintf(key, sizeof(key),
                                 "fonts/quranicFonts/qpc/p%d.ttf", page);
                        fid = g_fontManager.GetOrCreateFontId(
                            key, 16.0f * g_density, true);
                        bool found = false;
                        for (int p : pendingPages) {
                            if (p == page) {
                                found = true;
                                break;
                            }
    ...
    for (int i = 0; i < 5 && pendingIdx < (int)pendingPages.size();
         ++i, ++pendingIdx) {
        char key[64];
        snprintf(key, sizeof(key), "fonts/quranicFonts/qpc/p%d.ttf",
                 pendingPages[pendingIdx]);
        uint16_t fid = g_fontManager.GetFontId(key, 16.0f * g_density);
        if (fid != FontManager::kInvalidFontId &&
            g_fontManager.EnsureFontLoaded(fid)) {
            g_fontManager.Register(fid, g_clayRenderer);
    ...
    CLAY(CLAY_ID("QuranScrollContainer"),
         {
             .layout =
                 {
                     .sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)},
            ...
            for (int i = 0; i < g_totalAyahs; ++i) {
                ...
                CLAY_TEXT(ayahStrings[i],
                          CLAY_TEXT_CONFIG({
                              ...
                              .backgroundColor =
                                  isPlaying ? bg2 : (Clay_Color){0, 0, 0, 0},
                          }));

                CLAY(CLAY_IDI("translation", i),
                     {.layout = {.padding = {.top = 15, .bottom = 15}}}) {
                    CLAY_TEXT(
                        CLAY_STRING("test translation to be placed here this "
                                    "is just for test right no because i don't "
...
static float MeasureMushafWidth(const std::string& text, uint16_t fontId,
                                float fontSize) {
    ...
    if (!hbFont) return 0;
    hb_buffer_t* buf = hb_buffer_create();
    hb_buffer_add_utf8(buf, text.c_str(), (int)text.length(), 0,
                       (int)text.length());
...
static void LayoutQuranMushafContent() {
    ...
    if (s_lastVersion != g_surfaceVersion ||
    ...
    int64_t cacheKey =
        ((int64_t)g_selectedSurah << 32) | (uint32_t)g_extraMushafPages;
    ...
    if (it == s_cache.end()) {
        ...
        LOGI("Mushaf: surah %d pages %d-%d (extra=%d)", g_selectedSurah,
             startPage, endPage, g_extraMushafPages);

        std::unordered_set<int32_t> uniquePages;
        float availWidth =
            g_screenWidthDp - 12.0f - 12.0f - 15.0f - 15.0f - 1.0f;
        ...
        for (int32_t page = startPage; page <= endPage; ++page) {
            ...
            for (const auto& pl : layout.lines) {
                entry.lineSectionStart.push_back(
                    (int32_t)entry.sectionTexts.size());
                std::string fullText;
                int32_t ayahNum = 1;

                if (pl.lineType == "surah_name") {
                    const SurahInfo& sinfo =
                        g_quranDb.GetSurahInfo(pl.surahNumber);
                    ...
                    fullText =
                        ...
                        "\xD8\xA7\xD9\x84\xD8\xB1\xD9\x91\xD8\xAD\xD9\x92\xD9"
                        "\x85\xD9\x8E\xD9\x86\xD9\x90 "
                        "\xD8\xA7\xD9\x84\xD8\xB1\xD9\x91\xD8\xAD\xD9\x90\xD9"
                        "\x8A\xD9\x85\xD9\x90";
                    entry.sectionTexts.push_back(fullText);
                    entry.sectionAyahs.push_back(1);
                    entry.sectionSurahs.push_back(pl.surahNumber);
                    ...
                    for (int32_t wid = pl.firstWordId; wid <= pl.lastWordId;
                         ++wid) {
        ...
        for (int32_t page : uniquePages) {
            ...
            uint16_t fid =
                g_fontManager.GetOrCreateFontId(key, 16.0f * g_density, true);
            if (fid != FontManager::kInvalidFontId &&
                g_fontManager.EnsureFontLoaded(fid)) {
                g_fontManager.Register(fid, g_clayRenderer);
            }
        ...
        for (int32_t page : uniquePages) {
            ...
            for (float size = 28.0f; size >= 15.0f; size -= 0.5f) {
                ...
                for (int i = 0; i < entry.totalLines; ++i) {
                    ...
                    float w = MeasureMushafWidth(entry.lineTexts[i], pageFontId,
                                                 size);
                    if (w > availWidth) {
                        allFit = false;
                        break;
        ...
        LOGI(
            "MushafCache: surah %d extra=%d: build=%lldms font=%lldms "
            "fit=%lldms total=%lldms",
            g_selectedSurah, g_extraMushafPages,
            std::chrono::duration_cast<std::chrono::milliseconds>(tFont -
                                                                  tBuild)
                .count(),
            std::chrono::duration_cast<std::chrono::milliseconds>(tFit - tFont)
                .count(),
            std::chrono::duration_cast<std::chrono::milliseconds>(tEnd - tFit)
                .count(),
            std::chrono::duration_cast<std::chrono::milliseconds>(tEnd - tBuild)
                .count());
    ...
    for (const auto& s : g_mushafSectionTexts) {
        s_sectionStrings.push_back({.isStaticallyAllocated = false,
                                    .length = (int)s.length(),
                                    .chars = s.c_str()});
    }
    ...
    // ───────────────────────────────────────────────
    static std::vector<int32_t> s_orderedPages;
    s_orderedPages.clear();
    for (int i = 0; i < cache.totalLines; ++i) {
        int32_t p = cache.linePages[i];
        if (s_orderedPages.empty() || s_orderedPages.back() != p)
            s_orderedPages.push_back(p);
    }
    g_mushafPageCount = (int)s_orderedPages.size();

    // Clamp current page index to valid range
    if (g_mushafCurrentPage >= g_mushafPageCount)
        g_mushafCurrentPage = g_mushafPageCount - 1;
    if (g_mushafCurrentPage < 0) g_mushafCurrentPage = 0;

    // Each panel is the full available width inside the Root's 12dp left+right
    // padding. This must match the stride used by the touch handler.
    g_mushafPageStride = g_screenWidthDp - 24.0f;

    // Animate scroll toward the target page (lerp, skipped while finger is
    // down)
    if (!g_isSwiping) {
        g_mushafTargetX = g_mushafCurrentPage * g_mushafPageStride;
        float diff = g_mushafTargetX - g_mushafScrollX;
        g_mushafScrollX += diff * std::min(1.f, g_deltaTime * 12.f);
        if (std::fabs(diff) < 0.5f) g_mushafScrollX = g_mushafTargetX;
    }

    // ── Outer clip — full area, pages laid out horizontally
    // ───────────────────
    CLAY(CLAY_ID("MushafPagedOuter"),
         {
             .layout =
                 {
                     .sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)},
             ...
             .clip =
                 {
                     .horizontal = true,
                     .vertical = true,
                     .childOffset = {-g_mushafScrollX, 0},
                 },
         }) {
        // One full-screen-width panel per Mushaf page
        for (int pi = 0; pi < (int)s_orderedPages.size(); ++pi) {
            int32_t page = s_orderedPages[pi];

            // Page number label string — built per panel
            static std::vector<std::string> s_panelPageStrs;
            static std::vector<Clay_String> s_panelPageClayStrs;
            if ((int)s_panelPageStrs.size() <= pi) {
                s_panelPageStrs.resize(pi + 1);
                s_panelPageClayStrs.resize(pi + 1);
            }
            s_panelPageStrs[pi] = std::to_string(page);
            s_panelPageClayStrs[pi] = {
                .isStaticallyAllocated = false,
                .length = (int)s_panelPageStrs[pi].length(),
                .chars = s_panelPageStrs[pi].c_str()};

            CLAY(CLAY_IDI("MushafPagePanel", pi),
                 {
                     .layout =
                         {
                             // Each panel is exactly one stride wide so they
                             // tile perfectly in the horizontal strip.
                             .sizing = {CLAY_SIZING_FIXED(g_mushafPageStride),
                                        CLAY_SIZING_GROW(0)},
                             .padding = CLAY_PADDING_ALL(15),
                             .childGap = 8,
                             .childAlignment = {.x = CLAY_ALIGN_X_CENTER},
                ...
                // ──────────────────────────────────
                CLAY(CLAY_IDI("MushafPageLabel", pi),
                     {
                         .layout =
                             {
                                 .sizing = {CLAY_SIZING_GROW(0),
                    ...
                    CLAY_TEXT(s_panelPageClayStrs[pi],
                              CLAY_TEXT_CONFIG({
                                  .textColor = bg2,
                                  .fontId = g_roboto12,
                                  .fontSize = 12,
                ...
                // ───────────────────────────────────────
                for (int i = 0; i < cache.totalLines; ++i) {
                    if (cache.linePages[i] != page) continue;
             ...
             .layout =
                 {
                     ...
                     .padding = {.left = 12,
                                 .right = 12,
                                 .top =
                                     (uint16_t)(12 + (int)g_statusBarHeightDp),
                                 .bottom = 12},
                     .childGap = 12,
                     .layoutDirection = CLAY_TOP_TO_BOTTOM,
                 },
...
extern "C" JNIEXPORT void JNICALL
Java_com_primaveradev_alfalah_MainActivity_nativeOnDrawFrame(JNIEnv*, jobject) {
    Clay_SetPointerState(g_pointerPos, g_pointerDown);

    // Only run Clay's built-in vertical scroll tracking when not in paged
    // mushaf (mushaf drives its own offset via g_mushafScrollX)
    if (g_currentPage != Page::Quran ||
        g_quranDisplayMode != QuranDisplayMode::Mushaf) {
        if (!g_showAyahMenu)
            Clay_UpdateScrollContainers(true, (Clay_Vector2){0, 0},
                                        g_deltaTime);
    }
                ...
                CLAY(CLAY_ID("AyahMenu"),
                    ...
                    CLAY(
                        CLAY_ID("MenuPlayButton"),
                        {
                            ...
                            .image = {.imageData =
                                          ImageLoader::Get("images/play.png")},
                        }) {}
                    CLAY(
    ...
    // ── Auto-load more content when approaching the last loaded page/ayah ────
    if (g_currentPage == Page::Quran) {
        if (g_quranDisplayMode == QuranDisplayMode::Mushaf) {
            // Load 5 more pages when within 2 pages of the end
            if (g_mushafPageCount > 0 &&
                g_mushafCurrentPage >= g_mushafPageCount - 2) {
                LOGI(
                    "autoLoad mushaf: currentPage=%d pageCount=%d, loading 5 "
                    "more",
                    g_mushafCurrentPage, g_mushafPageCount);
                g_extraMushafPages += 5;
            }
        } else {
            Clay_ScrollContainerData data =
                Clay_GetScrollContainerData(CLAY_ID("QuranScrollContainer"));
            if (data.found && data.scrollPosition) {
                ...
                if (scrollBot >= data.contentDimensions.height - 200.0f) {
                    LOGI(
                        "autoLoad standard: scrollBot=%.0f contentH=%.0f "
                        "extra=%d",
                        scrollBot, data.contentDimensions.height,
    ...
    if (g_pointerDown && !g_pointerMovedSignificantly && !g_longPressFired) {
        ...
        if (elapsedMs >= LONG_PRESS_DURATION_MS) {
            ...
            if (g_currentPage == Page::Quran) {
                if (g_quranDisplayMode == QuranDisplayMode::Mushaf &&
                    !g_mushafSectionTexts.empty()) {
                    LOGI("long press mushaf: scanning %d commands",
                         commands.length);
                    for (int32_t ci = 0; ci < commands.length; ++ci) {
                        const Clay_RenderCommand& cmd =
                            commands.internalArray[ci];
                        if (cmd.commandType != CLAY_RENDER_COMMAND_TYPE_TEXT)
                            continue;
                        const auto& bb = cmd.boundingBox;
                        if (g_pointerPos.x >= bb.x &&
                            g_pointerPos.x < bb.x + bb.width &&
                            g_pointerPos.y >= bb.y &&
                            g_pointerPos.y < bb.y + bb.height) {
                            const char* ptr =
                                cmd.renderData.text.stringContents.chars;
                            for (int i = 0;
                                 i < (int)g_mushafSectionTexts.size(); ++i) {
                                const std::string& section =
                                    g_mushafSectionTexts[i];
                                if (!section.empty() && ptr >= section.data() &&
                                    ptr < section.data() + section.length()) {
                                    LOGI(
                                        "  -> matched mushaf section %d ayah "
                                        "%d",
                                        i, g_mushafSectionAyahs[i]);
                                    g_showAyahMenu = true;
                    ...
                    for (int32_t ci = 0; ci < commands.length; ++ci) {
                        ...
                        if (g_pointerPos.x >= bb.x &&
                            ...
                            const char* ptr =
                                cmd.renderData.text.stringContents.chars;
                            LOGI("  text cmd at (%.0f,%.0f) chars=%p len=%d",
                                 bb.x, bb.y, ptr,
                                 cmd.renderData.text.stringContents.length);
                            for (int i = 0; i < g_totalAyahs; ++i) {
                                const std::string& ayah = g_ayahTexts[i];
                                if (!ayah.empty() && ptr >= ayah.data() &&
                                    ptr < ayah.data() + ayah.length()) {
    ...
    if (g_wasPointerDown && !g_pointerDown) {
        double elapsedMs = (now - g_pointerDownTime) * 1000.0;
        bool isTap =
            !g_pointerMovedSignificantly && elapsedMs < TAP_MAX_DURATION_MS;
        ...
        if (g_showAyahMenu && isTap) {
            if (Clay_PointerOver(CLAY_ID("MenuPlayButton"))) {
                LOGI("play button tapped: surah=%d ayah=%d", g_menuSurah,
                     g_menuAyahNumber);
        ...
        // Normal navigation tap (only when menu is NOT active)
            ...
            if (isTap) {
                ...
                if (g_pendingTapActive &&
                    ...
                    if (selectedSurah > 0) g_selectedSurah = selectedSurah;
                } else {
                    // Buffer as a pending single tap — wait for double-tap
                    // window
